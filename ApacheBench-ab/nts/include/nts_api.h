...
// used to set the socket options value for any-type, any-state socket
...
ssize_t nts_sendfile(int out_fd, int in_fd, off_t *offset, size_t count);
...
int nts_raw_epoll_create(int size);
int nts_raw_epoll_ctl(int epfd, int op, int fd, struct epoll_event *event);
int nts_raw_epoll_wait(int epfd, struct epoll_event *events, int maxevents, int timeout);
int nts_raw_epoll_pwait(int epfd, struct epoll_event *events, int maxevents, int timeout, const __sigset_t *ss);
